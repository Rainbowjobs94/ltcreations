...
## 8) Capability Upgrade Matrix (Implementation-Ready)

### 8.1 Swift / iPhone Sensor Stack (Recommended)
- `CoreLocation` → GPS + movement geofencing.
- `CoreMotion` → orientation and inertial consistency checks.
- `DeviceCheck` / `AppAttest` → device integrity attestation.
- `CryptoKit` → local signing and secure key operations.
- `BackgroundTasks` → controlled periodic telemetry checks.

### 8.2 Web Frontend Stack
- React + TypeScript + Vite.
- Tailwind + component system.
- `zod` for request/response schema guards.
- `chart.js` for telemetry + escrow visualization.
- `playwright` for end-to-end attestation flow validation.

### 8.3 Deployment Scripts and CI/CD
...
- GitHub Actions jobs:
  ...
  - `security-scan`
  - `build-artifacts`
- Signed release artifacts and SBOM export (`cyclonedx`).

### 8.4 Test Suite Matrix
- Backend Python: `unittest`/`pytest` + hypothesis fuzzing for spoofing edge cases.
- Rust services: `cargo test`, `proptest`, `cargo-audit`.
- Solidity contracts: Foundry (`forge test`) + invariant tests.
- Frontend: Vitest + Testing Library + Playwright.

### 8.5 Quickstart Milestones
1. Implement canonical attestation API endpoint.
2. Add replay-cache and timestamp freshness checks.
3. Add spoofing fixtures (GPS jump, UV mismatch, impossible compass drift).
4. Wire dashboard and infographic to live test outputs.
5. Gate block creation on attestation verdict + reason codes.
